...
#' @return Either an \code{\link{ExpressionSet} object,
#' \code{\link{GRangesList}}) object, or data frame (when no clinical data requested for the selected data type or when clinical data is exclusively requested).
  ...
  if (dim(dm)[1] == 0 | dim(dm)[2] == 0) {
    ...
    if (slotreq %in% c("Methylation", "AllByGene", "ThresholdedByGene")) {
      ...
      if (isCharRow) {
        ...
        rNames <- geneSymbols
        rNames <- ifelse(grepl("|", rNames, fixed = TRUE),
                                strsplit(rNames, "\\|")[[1]][1], rNames)
      } else {
      rNames <- rownames(dm)
      }
