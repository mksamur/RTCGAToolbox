...
extract <- function(object, type, clinical = TRUE) {
  ...
  if(clinical){
    ...
    filler <- substr(rownames(pd)[1], 5, 5)
    if (filler != "-") {
      rownames(pd) <- gsub(paste0("\\", filler), "\\-", rownames(pd))
    }
    if(length(pd)==0){
      stop("No clinical data available!")
    }
    clinextra <- .dlClinx(object)
    filler <- substr(rownames(clinextra)[1], 5, 5)
    if (filler != "-") {
      rownames(clinextra) <- gsub(paste0("\\", filler), "\\-", rownames(clinextra))
    }
